...
def get_best_pixel(pixel_list):
    ...
    for pixel in pixel_list:
        ...
        blue_avg += pixel.blue
    red_avg = red_avg // len(pixel_list)
    green_avg = green_avg // len(pixel_list)
    blue_avg = blue_avg // len(pixel_list)
    dist = []
    for i in range (len(pixel_list)):
        dist.append(get_pixel_dist(pixel_list[i], red_avg, green_avg, blue_avg))

    min_index = dist.index(min(dist))
    return pixel_list[min_index]
...
def create_ghost(image_list):
    ...
    for pixel in image:
        ...
        for count in range(number_of_images):
            pixel_list.append(image_list[count].get_pixel(x, y))
        image.set_pixel(x, y, get_best_pixel(pixel_list))
