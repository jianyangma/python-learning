...
def convert_rating_to_index(rating):
    ...
    2
    """
    if rating < LOW_BUCKET:
        return 0
    elif rating > MED_BUCKET:
        return 2
    else:
        return 1
...
def add_data_for_word(word_data, word, gender, rating):
    ...
    """
    inner_dict = {KEY_WOMEN: [0, 0, 0], KEY_MEN: [0, 0, 0]}
    bucket = convert_rating_to_index(rating)
    if word in word_data:
        if gender == "M":
            cur = word_data[word][KEY_MEN][bucket]
            word_data[word][KEY_MEN][bucket] = cur + 1
        else:
            cur = word_data[word][KEY_WOMEN][bucket]
            word_data[word][KEY_WOMEN][bucket] = cur + 1
    else:
        if gender == "M":
            inner_dict[KEY_MEN][bucket] = 1
            word_data[word] = inner_dict
        else:
            inner_dict[KEY_WOMEN][bucket] = 1
            word_data[word] = inner_dict
